...
func (q *Queries) UsersGetAllUsernames(ctx context.Context) ([]string, error) {
...
const usersGetUsernamesCount = `-- name: UsersGetUsernamesCount :one
...
func (q *Queries) UsersGetUsernamesCount(ctx context.Context) (int64, error) {
	row := q.queryRow(ctx, q.usersGetUsernamesCountStmt, usersGetUsernamesCount)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const usersGetUsernamesPaginated = `-- name: UsersGetUsernamesPaginated :many
...
type UsersGetUsernamesPaginatedParams struct {
	Offset int32
	Limit  int32
}

func (q *Queries) UsersGetUsernamesPaginated(ctx context.Context, arg UsersGetUsernamesPaginatedParams) ([]string, error) {
	rows, err := q.query(ctx, q.usersGetUsernamesPaginatedStmt, usersGetUsernamesPaginated, arg.Offset, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
