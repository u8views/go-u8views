...
ssh:
	# cat ~/.ssh/id_ed25519.pub | ssh root@64.176.71.11 "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys"
	ssh -t root@64.176.71.11 "cd /var/go/u8views/; bash --login"

ssh-copy-tls-certificates:
	mkdir -p ./.docker/volumes/go/tls-certificates
	scp -r root@64.176.71.11:/var/go/u8views/.docker/volumes/go/tls-certificates ./.docker/volumes/go
