...
SELECT user_id,
       COALESCE(SUM(count) FILTER ( WHERE time >= $1 ), 0)::BIGINT AS hour_count,
       COALESCE(SUM(count) FILTER ( WHERE time >= $2 ), 0)::BIGINT  AS day_count,
       COALESCE(SUM(count) FILTER ( WHERE time >= $3 ), 0)::BIGINT AS week_count,
       SUM(count)                                                     AS month_count
FROM profile_hourly_views_stats
WHERE user_id = ANY ($4::BIGINT[])
  AND time >= $5
GROUP BY user_id
`
